...
let canvas, ctx, epochEl, environmentEl, annDiv;
let mrEl, mrVal, initPopEl, initVal, reproEl, reproVal, sexualToggle;
let btnRestart, btnPause, btnSpawnFood, btnTriggerEvent;
let speedSlider, speedValue, btnSlow, btnNormal, btnFast;
let stats, chartCtx, chart;
let sim, ANIM;

// MOBILE COMPATIBILITY FIXES
console.log("EvoVerse loading...");

// Fix: Wait for DOM to be fully ready
...
function initializeApp() {
    console.log("Initializing EvoVerse application...");
    
    // Initialize global variables after DOM is ready
    initializeGlobalVariables();
    
    // Set up mobile adjustments
    if (isMobileDevice()) {
...
function initializeGlobalVariables() {
    ...
    // UI elements
    epochEl = document.getElementById('epochName');
    environmentEl = document.getElementById('environmentName');
    annDiv = document.getElementById('announcements');
    
    // Control elements
    mrEl = document.getElementById('mutationRate');
    mrVal = document.getElementById('mr_val');
    initPopEl = document.getElementById('initPop');
    initVal = document.getElementById('init_val');
    reproEl = document.getElementById('reproEnergy');
    reproVal = document.getElementById('repro_val');
    sexualToggle = document.getElementById('sexualToggle');
    btnRestart = document.getElementById('btnRestart');
    btnPause = document.getElementById('btnPause');
    btnSpawnFood = document.getElementById('btnSpawnFood');
    btnTriggerEvent = document.getElementById('btnTriggerEvent');
    ...
    if (miniChart) {
        chartCtx = miniChart.getContext('2d');
...
function applyMobileFixes() {
    ...
    // Replace click events with touch events for mobile
    if (btnRestart) {
    ...
    // Add error reporting for mobile
...
/* ----------------- BOOT & INITIALIZATION ----------------- */
function boot() {
  console.log("Booting EvoVerse simulation...");
  ...
  if (!canvas) {
    ...
    initializeGlobalVariables();
    if (!canvas) {
        console.error("Still no canvas. Aborting.");
        return;
    }
  }
  
  // Set canvas size (might have been set by mobile fixes)
  if (canvas.width === 0) {
      canvas.width = 960;
      canvas.height = 720;
  }
...
// SINGLE restartSim function (no duplicates)
function restartSimulation() {
  ...
  if (epochEl) epochEl.innerText = sim.epoch;
  if (environmentEl) environmentEl.innerText = 'Normal';
  
  console.log("Simulation ready - Population:", sim.agents.length, "Food:", sim.foods.length);
}

function initUI() {
  console.log("Setting up UI controls...");
  
  // Your existing initUI code here...
  if (mrEl) {
  ...
  if (btnExportMetrics) {
    btnExportMetrics.addEventListener('click', () => { exportMetricsCSV(); });
  }
  
  const btnExportSpecies = document.getElementById('btnExportSpecies');
  if (btnExportSpecies) {
    btnExportSpecies.addEventListener('click', () => { exportSpeciesCSV(); });
  }
  
  const btnExportLineage = document.getElementById('btnExportLineage');
  if (btnExportLineage) {
    btnExportLineage.addEventListener('click', () => { exportLineageJSON(); });
  }
...
function stepLoop() {
  ...
  if (ANIM.running && stepsToRun > 0) {
    for (let i = 0; i < stepsToRun; i++) {
      sim.stepSim(sexualToggle ? sexualToggle.checked : false);
    }
